...
void main() {
  group('自動戦闘', () {
    test('初期状態でステージ1に勝てる', () {
      final result = Battle(organs: party(), stage: 1).run();
      expect(result.won, isTrue, reason: '始めたばかりの人が1戦目で詰むと続かない');
    });

    test('同じ状態なら結果は必ず同じ', () {
      final a = Battle(organs: party(), stage: 3).run();
      final b = Battle(organs: party(), stage: 3).run();
    ...
    test('健康なほうが強い', () {
      final healthy = Battle(organs: party(health: 100), stage: 5).run();
      final weak = Battle(organs: party(health: 20), stage: 5).run();
    ...
    test('サボり続けると勝てなくなる', () {
      final neglected = Battle(organs: party(health: 20), stage: 12).run();
      expect(neglected.won, isFalse);
    });

    test('レベルを上げれば同じ健康度でも早く倒せる', () {
      const stage = 14;
      final low = Battle(organs: party(health: 60), stage: stage).run();
      final high = Battle(organs: party(health: 60, level: 10), stage: stage).run();

      expect(high.turns, lessThan(low.turns));
    });

    test('必ず決着するか、上限ターンで打ち切られる', () {
      final result = Battle(organs: party(health: 20), stage: 40).run();
